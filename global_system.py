...
def combine_scores(tfidf_scores, bm25_scores, embedding_scores, alpha=0.1, beta=0.5, gamma=0.4):
    """
    Combine les scores de TF-IDF, BM25 et Embeddings avec une pondération et renvoie
    les documents triés par ordre de pertinence.

    Args:
    - tfidf_scores (dict): Dictionnaire des scores TF-IDF {index: score}.
    ...
    - alpha (float): Pondération pour TF-IDF.
    - beta (float): Pondération pour BM25.
    - gamma (float): Pondération pour les embeddings sémantiques.

    Returns:
    - list: Liste des indexes triés par score de pertinence décroissant.
    """
    # Combine les trois systèmes de notation dans un seul dictionnaire
    combined_scores = {}

    # Fusionner les scores en utilisant les poids alpha, beta et gamma
    for doc_id in set(tfidf_scores.keys()).union(bm25_scores.keys()).union(embedding_scores.keys()):
        tfidf_score = tfidf_scores.get(doc_id, 0)   # Si l'index est manquant, on suppose un score de 0
        bm25_score = bm25_scores.get(doc_id, 0)
        embedding_score = embedding_scores.get(doc_id, 0)

        # Calcul du score combiné
        combined_scores[doc_id] = (alpha * tfidf_score) + (beta * bm25_score) + (gamma * embedding_score)
